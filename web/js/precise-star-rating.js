function rateMedia(mediaId, rate, numStar, starWidth) {
  ...
  $.ajax({ // JQuery Ajax
    ...
    success: function(response) {
      $.cookie("symfonyRatingSystem" + mediaId, "Rated", {
        expires: 1
      }); // Add jQuery Cookie Plugin to use this function
      $('.box' + mediaId).html('<div style="font-size: small; color: green">Merci pour votre note</div>'); // Return "Thank you for rating"
      // We update the rating score and number of rates
      $('.resultMedia' + mediaId).html('<div style="font-size: small;  grey">Rating: ' + response.avg + '/' + numStar + ' (' + response.nbrRate + ' votes)</div>');
